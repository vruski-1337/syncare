...
<tbody>@foreach($products as $p)<tr><td>{{ $p->id }}</td><td>{{ $p->name }}</td><td>{{ $p->price }}</td><td>{{ $p->quantity }}</td><td>{{ $p->unit }}</td><td><a href="{{ route('products.show',$p) }}">View</a> <a href="{{ route('products.edit',$p) }}">Edit</a> <form method="POST" action="{{ route('products.destroy',$p) }}" style="display:inline">@csrf @method('DELETE')<button class="text-red-500" onclick="return confirm('Delete?')">Del</button></form></td></tr>@endforeach</tbody>
